...
#'ct_dst_sport_ltm', 'ct_dst_src_ltm', 'swin',
...
class pwlModel:
    ...
    def evaluate(self, X_train, X_test, y_train, y_test):
        ...
        y_proba_train = sigmoid(self.r_func(np.dot(X_train, self.weights1), np.dot(X_train, self.weights2)))
        y_proba_test = sigmoid(self.r_func(np.dot(X_test, self.weights1), np.dot(X_test, self.weights2)))
    ...
    def save_model(self, path="models/PWL_model.pkl"):
        ...
        joblib.dump({"weights1": self.weights1, "weights2": self.weights2, "scaler": self.scaler, "pca": self.pca}, path)
